...
2. Check if `ridl/prompts/` already has template files
   - **No existing files:** Generate all 6 default templates
...
The skill generates 6 Liquid template files. These are the canonical defaults — a RIDL harness reads these at runtime and fills in the Liquid variables with data from `ridl.json` and runtime state.
...
2. Read `progress.md` and `learnings.md` if they exist (check Codebase Patterns section first)
...
7. Append your progress to `progress.md` and learnings to `learnings.md`
...
### 4. `learnings_report.liquid`

Defines the format for capturing learnings and institutional knowledge. The agent appends to `learnings.md` after each iteration with patterns, gotchas, and useful context for future iterations.
...
## Learnings Report Format

APPEND to learnings.md (never replace, always append):
...
- [ ] All 6 template files generated:
  - [ ] `agent_instructions.liquid`
  - [ ] `story_context.liquid`
  - [ ] `progress_report.liquid`
  - [ ] `learnings_report.liquid`
